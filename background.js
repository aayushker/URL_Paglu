...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  ...
  if (changeInfo.status === 'complete' && tab.url && 
      tab.url.startsWith('http') && !tab.url.includes('chrome.google.com')) {
    ...
    chrome.storage.sync.get({ autoScan: false }, function(settings) {
      if (settings.autoScan) {
        ...
        chrome.tabs.sendMessage(tabId, { action: 'performScan' }, response => {
          const error = chrome.runtime.lastError;
          if (error) {
            debug(`Error sending message to content script: ${error.message}`);
          } else if (response) {
            debug(`Auto-scan completed with ${response.stats?.suspicious || 0} suspicious URLs`);
          }
        });
      } else {
